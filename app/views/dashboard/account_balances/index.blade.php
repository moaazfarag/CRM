        ...
        <div ng-app="accountsApp" ng-controller="accountController" class="card">
            ...
            <form ng-submit='submitItem()' name='form'>
                ...
                <div class="title">
                    ...
                    @if(PerC::isShow('balances','accountsBalances','show'))
                        <a style="float: left;height:30px;line-height:32px;font-size: medium" type="button"
                           href="{{ URL::route('viewAccountsBalances') }}" class="btn btn-small z-depth-0">
                            عرض أرصدة الحسابات
                        </a>
                    @endif
                </div>
                @if(PerC::isShow('balances','accountsBalances','add'))
                    <div class="content">
                        <div class="row no-margin-top">
                        ...
                        <div class="row">
                            <div class="col s12 l11">
                                <div class="input-field">
                                    <i class="mdi-communication-chat prefix"></i>
                                    <textarea ng-model='account.note' name="notes" id="notes"
                                              class="materialize-textarea" length="120"> {{ @$item->notes }}</textarea>
                                    <?php $notes = Lang::get('main.notes') ?>
                                    {{ Form::label('notes',$notes) }}
                                    <p class="parsley-required">{{ $errors ->first('notes') }} </p>
                                </div>
